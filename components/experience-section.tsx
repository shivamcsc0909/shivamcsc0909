...
export function ExperienceSection() {
  return (
    <section id="experience" className="relative w-full py-24 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-primary/5 blur-[140px] pointer-events-none rounded-full" />
      ...
      <div className="mx-auto max-w-5xl relative z-10">
        ...
        >
          {experiences.map((exp, idx) => (
            <motion.div
              ...
              whileHover={{ y: -3 }}
              className="group relative rounded-3xl border border-border/80 bg-card/85 p-7 sm:p-9 backdrop-blur-xl shadow-lg transition-all duration-300 hover:border-primary/50 hover:shadow-2xl"
            >
              <div className="space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/70 pb-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {exp.type && (
                        <span className="rounded-full bg-primary/15 px-2.5 py-0.5 text-[11px] font-bold text-primary border border-primary/20">
                          {exp.type}
                        </span>
                      )}
                    </div>
                  ...
                  <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary bg-primary/10 px-3.5 py-1.5 rounded-full border border-primary/20 w-fit">
                ...
                {exp.techStack && exp.techStack.length > 0 && (
                  <div className="pt-4 border-t border-border/60 flex flex-wrap gap-1.5 items-center">
                    <span className="text-[11px] font-semibold text-muted-foreground mr-1">Stack:</span>
                    {exp.techStack.map((tech) => (
                      <span
                        key={tech}
                        className="rounded-lg bg-background/90 px-2.5 py-1 text-[11px] font-medium text-foreground border border-border/70"
                      >
                        {tech}
                      </span>
                    ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <motion.div
            ...
            transition={{ duration: 0.5 }}
            viewport={{ once: true }}
            className="lg:col-span-6 rounded-3xl border border-border/80 bg-card/85 p-7 sm:p-9 backdrop-blur-xl shadow-lg space-y-6"
          >
            <div className="flex items-center gap-3 border-b border-border/70 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Computer science & engineering foundation</p>
              </div>
            </div>
            ...
            <div className="space-y-5">
              {education.map((edu, eIdx) => (
                <div
                  key={eIdx}
                  className="rounded-2xl border border-border/60 bg-background/50 p-5 space-y-2 transition-all hover:border-primary/40"
                >
                  <div className="flex justify-between items-start gap-2">
                    <h4 className="font-extrabold text-foreground text-sm sm:text-base">{edu.degree}</h4>
                    <span className="text-xs font-semibold text-primary bg-primary/10 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                      {edu.period}
                    </span>
                  </div>
                  ...
                  {edu.coursework && edu.coursework.length > 0 && (
                    <div className="pt-2 border-t border-border/40">
                      <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Core Curriculum:</p>
          ...
          >
            <div className="flex items-center gap-3 border-b border-border/70 pb-4">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-accent/15 text-accent shadow-sm">
                <Award className="h-5 w-5" />
              </div>
              <div>
                <h3 className="text-2xl font-bold text-foreground">Verified Certifications</h3>
                <p className="text-xs text-muted-foreground">AWS, Meta, Docker & enterprise specializations</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {certifications.map((cert, cIdx) => (
                <div
                  key={cIdx}
                  className="flex flex-col justify-between rounded-2xl border border-border/60 bg-background/50 p-4 transition-all hover:border-primary/40 space-y-2"
                >
